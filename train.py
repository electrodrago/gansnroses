...
def test(args, genA2B, genB2A, testA_loader, testB_loader, name, step):
    ...
    with torch.no_grad():
        ...
        wandb.log({'A2B':wandb.Image(f'{im_path}/{name}_A2B_{str(step).zfill(6)}.jpg')})
        wandb.log({'B2A':wandb.Image(f'{im_path}/{name}_B2A_{str(step).zfill(6)}.jpg')})
...
def train(args, trainA_loader, trainB_loader, testA_loader, testB_loader, G_A2B, G_B2A, D_A, D_B, G_optim, D_optim, device):
    ...
    for idx in pbar:
        ...
        if get_rank() == 0:
            ...
            wandb.log({'Epoch': i, 'D_adv_loss_value': D_adv_loss_val, 'lpips':lpips_val, 'G_adv_loss_value': G_adv_loss_val, 'Gcycle_loss_value': G_cycle_loss_val, 'G_style_loss':G_style_loss.item()})
